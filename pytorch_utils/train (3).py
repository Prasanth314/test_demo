...
# Learning parameters
checkpoint = "/content/CycleGAN-PyTorch/checkpoint_srresnet.pth.tar"  # path to model checkpoint, None if none
...
# optimizer_sr = checkpoint['optimizer']
# for g in optimizer_sr.param_groups:
#     g['lr'] = 1e-6
optimizer_sr = torch.optim.Adam(params=filter(lambda p: p.requires_grad, model.parameters()),
                              lr=1e-6)
...
for epoch in range(0, args.epochs):
    ...
    for i, data in progress_bar:
        ...
        ##############################################
         ...
         # Super Resolution loss between day and back translated day
        
        lr_imgs = F.interpolate(recovered_image_A.detach(), size=128,mode='bilinear')
        sr_imgs = model(lr_imgs)
        print("fake_image_B:",recovered_image_A.size())
        print("sr_imgs:",sr_imgs.size())
        loss_sr_day = criterion(sr_imgs, real_image_A)

         # Super Resolution loss between night and back translated night
        lr_imgs = F.interpolate(recovered_image_B.detach(), size=128,mode='bilinear')
        sr_imgs = model(lr_imgs)
        print("fake_image_B:",recovered_image_B.size())
        print("sr_imgs:",sr_imgs.size())
        loss_sr_night = criterion(sr_imgs, real_image_B)

        loss_sr = loss_sr_day + loss_sr_night
        ...
        if grad_clip is not None:
            clip_gradient(optimizer_sr, grad_clip)
        optimizer_sr.step()
        ...
        if i % args.print_freq == 0:
            ...
            vutils.save_image(fake_image_B.detach(),
                              ...
                              normalize=True)
            print("Super Resol Loss:",loss_sr)
